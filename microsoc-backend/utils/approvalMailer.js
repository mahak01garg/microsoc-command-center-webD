...
async function sendIncidentAssignmentEmail({ user, incident, assignedBy, note }) {
  if (!user?.email || !incident) {
    return { skipped: true, reason: 'Missing assignment email recipient or incident' };
  }

  const incidentUrl = `${getFrontendUrl()}/#/incidents`;
  const subject = `MicroSOC incident assigned: ${incident.title || 'Security Incident'}`;
  const text = [
    `Hi ${user.name || 'there'},`,
    ``,
    `A MicroSOC incident has been assigned to you.`,
    ``,
    ...
    note ? `Note: ${note}` : '',
    ``,
    `Open MicroSOC: ${incidentUrl}`
  ].filter(Boolean).join('\n');
  const html = `
    <div style="font-family:Arial,sans-serif;line-height:1.5;color:#111827">
      <h2>MicroSOC Incident Assigned</h2>
      <p>Hi ${user.name || 'there'},</p>
      <p>A MicroSOC incident has been assigned to you.</p>
      <ul>
        <li><strong>Incident:</strong> ${incident.title || 'Security Incident'}</li>
        <li><strong>Severity:</strong> ${incident.severity || 'N/A'}</li>
        <li><strong>Status:</strong> ${incident.status || 'open'}</li>
        <li><strong>Assigned by:</strong> ${assignedBy || 'Admin'}</li>
        ${note ? `<li><strong>Note:</strong> ${note}</li>` : ''}
      </ul>
      <p>
        <a href="${incidentUrl}" style="display:inline-block;padding:10px 14px;background:#0ea5e9;color:white;text-decoration:none;border-radius:6px">Open Incidents</a>
      </p>
    </div>
  `;
  ...
  return sendEmailOrLog({
    ...
    html,
    text,
    fallbackLog: () => console.warn(`Incident assignment email for ${user.email}: ${incident.title || incident._id}`)
  });
}

async function sendCriticalAlertEmail({ alert, log }) {
  const subject = `MicroSOC critical alert: ${alert?.title || 'Critical Detection'}`;
  const dashboardUrl = `${getFrontendUrl()}/#/alerts`;
  const text = [
    `Critical MicroSOC alert detected.`,
    ``,
    `Title: ${alert?.title || 'Critical Detection'}`,
    `Attack Type: ${alert?.attackType || log?.attackType || 'Unknown'}`,
    `Source IP: ${alert?.sourceIP || log?.sourceIP || 'Unknown'}`,
    `Target: ${alert?.targetSystem || log?.targetSystem || 'Unknown'}`,
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;line-height:1.5;color:#111827">
      <h2>MicroSOC Critical Alert</h2>
      <p>A critical alert was generated by MicroSOC.</p>
      <ul>
        <li><strong>Title:</strong> ${alert?.title || 'Critical Detection'}</li>
        <li><strong>Attack Type:</strong> ${alert?.attackType || log?.attackType || 'Unknown'}</li>
        <li><strong>Source IP:</strong> ${alert?.sourceIP || log?.sourceIP || 'Unknown'}</li>
        <li><strong>Target:</strong> ${alert?.targetSystem || log?.targetSystem || 'Unknown'}</li>
        <li><strong>Severity:</strong> ${alert?.severity || 'critical'}</li>
      </ul>
      <p>
        <a href="${dashboardUrl}" style="display:inline-block;padding:10px 14px;background:#dc2626;color:white;text-decoration:none;border-radius:6px">Open Alerts</a>
      </p>
    </div>
  `;
  ...
  return sendEmailOrLog({
    ...
    html,
    text,
    fallbackLog: () => console.warn(`Critical alert email: ${alert?.title || log?.attackType || 'Critical Detection'}`)
  });
}
