...
async function sendEmailOrLog({ to, subject, html, text, fallbackLog }) {
  ...
  if (normalizedSubject.includes('alert')) {
    console.warn(`⚠️ Email skipped: alert mail blocked (${subject})`);
    if (fallbackLog) fallbackLog();
    return { sent: false, skipped: true, reason: 'Alert emails are disabled' };
  }
...
async function sendCriticalAlertEmail({ alert, log }) {
  ...
  return {
    skipped: true,
    sent: false,
    reason: 'Alert emails are disabled for live detections'
  };
}
